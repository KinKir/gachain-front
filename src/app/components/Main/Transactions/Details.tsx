            ...
            <div style={{ textAlign: 'center', color: 'rgb(153, 153, 153)' }}>
                {this.props.flowData === null ? <FormattedMessage id={`record.${this.props.detailsValue}`} defaultMessage={this.props.detailsMessage} /> :
                    this.props.flowData.map((item, k) => (
                        <Row key={k} style={{ textAlign: 'left', marginTop: '20px' }}>
                            ...
                            <div style={{ margin: '20px 40px' }}>
                                <Row>
                                    <span style={{ color: '#262626' }}>
                                        <FormattedMessage id="record.hash" defaultMessage="Transaction hash" />:
                                    </span>
                                    <StyledHash style={{ marginLeft: '4px' }}>
                                        <a href={`https://devexplore.gac.one/#/transaction/${item.txhash}`} target="_blank">{item.txhash}</a>
                                    </StyledHash>
                                </Row>
                                <Row>
                                    <span style={{ color: '#262626' }}><FormattedMessage id="record.time" defaultMessage="Generation time" />:</span>
                                    <span style={{ marginLeft: '4px' }}>{moment(item.created_at).format()}</span>
                                </Row>
                                <Row><span style={{ color: '#262626' }}><FormattedMessage id="record.utcTime" defaultMessage="UTC time" />:</span>
                                    <span style={{ marginLeft: '10px' }}>{moment(item.created_at).utc().format()}</span>
                                </Row>
                                <Row>
